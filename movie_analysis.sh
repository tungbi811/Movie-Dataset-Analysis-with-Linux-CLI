...
# Q0. Merge multiple line records and remove commas inside quotes
(
 awk '
  /^[0-9]+,/{
   print ""
...
(
 ...
 tail -n +2 movies_cleaned.csv | awk -F',' '$NF != 0 { print $NF "\t" $0}' | sort -k1,1nr | cut -f2- | head -1
 
 tail -n +2 movies_cleaned.csv | awk -F',' '$NF != 0 { print $NF "\t" $0}' | sort -k1,1nr | cut -f2- | tail -1
) > q3.csv

# Q4. Total revenue of all movies
(
 echo "Total revenue of all movies"
 tail -n +2 movies_cleaned.csv | awk -F',' '{sum += $NF}; END{print sum}' movies_cleaned.csv
) > q4.csv
...
(
 echo "keyword,count"
 tail -n +2 movies_cleaned.csv| awk -F',' '$11 != "" {print $11}' | tr "|" "\n" | sort | uniq -c | sort -k1r | awk '{print $2,$1}'
) > q10.csv
